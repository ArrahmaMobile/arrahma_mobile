/// Shared models for the arrahma app.
///
/// Used by the mobile app and the scraper projects.
library models;

export 'src/app_metadata.dart';
export 'src/course_item.dart';
export 'src/quran_course.dart';
export 'src/broadcast_item.dart';
export 'src/course_registration.dart';
export 'src/heading_banner.dart';
export 'src/quran_course_tafseer.dart';
export 'src/quran_course_tajweed.dart';
export 'src/quran_course_test.dart';
export 'src/social_media_item.dart';
export 'src/surah.dart';
